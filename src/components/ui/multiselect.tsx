...
const MultipleSelector = React.forwardRef<MultipleSelectorRef, MultipleSelectorProps>(({ value, onChange, placeholder, defaultOptions: arrayDefaultOptions = [], options: arrayOptions, delay, onSearch, onSearchSync, loadingIndicator, emptyIndicator, maxSelected = Number.MAX_SAFE_INTEGER, onMaxSelected, hidePlaceholderWhenSelected, disabled, groupBy, className, badgeClassName, selectFirstItem = true, creatable = false, triggerSearchOnFocus = false, commandProps, inputProps, hideClearAllButton = false }: MultipleSelectorProps, ref: React.Ref<MultipleSelectorRef>) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex flex-nowrap gap-1 overflow-x-auto scrollbar-hide pr-8 pl-0.5">
          {selected.map((option) => {
            return (
              <div key={option.value} className={cn('animate-fadeIn relative inline-flex h-7 flex-shrink-0 cursor-default items-center rounded-md border border-solid bg-background pe-7 pl-2 ps-2 text-xs font-medium text-secondary-foreground transition-all hover:bg-background disabled:cursor-not-allowed disabled:opacity-50 data-[fixed]:pe-2 shadow-sm', badgeClassName)} data-fixed={option.fixed} data-disabled={disabled || undefined}>
                {option.label}
                <button
          ...
          <CommandPrimitive.Input
            ...
            className={cn(
              'flex-1 bg-transparent outline-none placeholder:text-muted-foreground disabled:cursor-not-allowed',
              {
                ...
                'ml-1 min-w-[80px]': selected.length !== 0,
              },
          ...
          <button
            ...
            className={cn('absolute end-0 top-0 size-9 items-center justify-center rounded-lg border border-transparent text-muted-foreground/80 transition-colors hover:text-foreground focus-visible:outline focus-visible:outline-ring/70 hidden', (hideClearAllButton || disabled || selected.length < 1 || selected.filter((s) => s.fixed).length === selected.length) && 'hidden')}
      ...
      <div className="relative">
        <div className={cn('absolute top-2 z-10 w-full overflow-hidden rounded-lg border border-input', 'data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95', !open && 'hidden')} data-state={open ? 'open' : 'closed'}>
          {open && (
            <CommandList
              className="!bg-white text-popover-foreground shadow-lg shadow-black/5 outline-none"
              onMouseLeave={() => {
